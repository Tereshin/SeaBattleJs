(function($) {
	 ...
	 */
	Game.Arena = function(body, score, status, sizeX, sizeY, debug) {
		this.sizeX = (sizeX || 10);
		this.sizeY = (sizeY || 10);
		...
		*/
		 ...
		 */
		this.score = new Game.Score(this.shipTypes, this.scoreArea);
	...
	Game.Arena.prototype = {
		 ...
		 * @protected
		 * @param {Game.Ship} ship Ship object
		 * @param {Array.<Number, Array.<Number>>} shipsMap thwo-dimensional array of arena ship map
		...
		makeShipsMap : function(shipTypes) {
			...
			for (type in shipTypes) {
				...
				for (var i = 0; i < count; i++) {
					var ship = new Game.Ship(type, size);
